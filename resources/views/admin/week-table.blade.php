  ...
  <div class="content-wrapper">
    ...
    <div class="card">
            ...
            <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
               ...
               <a href="/display-week-form" button type="button" class="btn btn-primary">Add Week</button></a>
               @endif
            </div>
                </div>
            </div>
            ...
            <div class="card-body table-responsive no-padding">
              <table  class="table table-bordered table-hover">
                ...
                <tr>
                  <th>No</th>
                  <th>Week</th>
                  @if(in_array('Can see week created', auth()->user()->getUserPermisions()))
                  <th>Created By</th>
                  @endif
                  @if(in_array('Can see week action', auth()->user()->getUserPermisions()))
                  <th>Action</th>
                  @endif
                </tr>
                </thead>
                ...
                @if ($show_all_weeks->currentPage() > 1)
                      ...
                      @php($i = 1)
                      @endif
              @foreach ($show_all_weeks as $index =>$weeks)
                  <tr>
                      <td>{{ $i++ }}</td>
                      <td>{{ $weeks->week }}</td>
                      @if(in_array('Can see week created by name', auth()->user()->getUserPermisions()))
                      <td>{{ $weeks->name }}</td>
                      @endif
              ...
              {{ $show_all_weeks->appends(['name' => $search_query])->links() }}
              @else
              {{ $show_all_weeks->links() }}
              @endif
            </div>
            </div>
            <!-- /.card-body -->
