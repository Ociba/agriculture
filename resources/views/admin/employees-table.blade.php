  ...
  <div class="content-wrapper">
    ...
    <div class="card">
            ...
            <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
               ...
               <a href="/display-employees-form" button type="button" class="btn btn-primary">Add Employee</button></a>
               @endif
            </div>
                </div>
            </div>
            ...
            <div class="card-body table-responsive no-padding">
              <table  class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <th>Qualification</th>
                  <th>Experience</th>
                  @if(in_array('Can see employees action', auth()->user()->getUserPermisions()))
                  <th>Action</th>
                  @endif
                </tr>
                </thead>
                ...
                @if ($display_employees->currentPage() > 1)
                      ...
                      @php($i = 1)
                      @endif
              @foreach ($display_employees as $index =>$employee)
                  <tr>
                      <td>{{ $i++ }}</td>
                      <td>{{ $employee->name }}</td>
                      ...
                      <td>
                      <a href="/edit-employee/{{ $employee->id }}" data-widget="edit" data-toggle="tooltip" title="edit">
                      <span style="color:blue;"><i class="fa fa-edit"></i></span></a>
                      @endif
                      @if(in_array('Can delete employee', auth()->user()->getUserPermisions()))
                      <a href="/delete-employee/{{ $employee->id }}" data-widget="deny" data-toggle="tooltip" title="delete">
              ...
              {{ $display_employees->appends(['name' => $search_query])->links() }}
              @else
              {{ $display_employees->links() }}
              @endif
            </div>
            </div>
            <!-- /.card-body -->
