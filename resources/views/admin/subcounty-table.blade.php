                        ...
                        <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <a href="/display-subcounty-form" button type="button" class="btn btn-primary">Add Subcounty</button></a>
                                    @endif
                                    </h4>
                                </div>
                            </div>
                                    ...
                                    <div class="card-body card-dashboard dataTables_wrapper dt-bootstrap table-responsive">
                                        <table class="table table-bordered table-hover file-export">
                                            <thead>
                                                <tr>
                                                <th>Sub County</th>
                                                  @if(in_array('Can see subcounty action', auth()->user()->getUserPermisions()))
                                                  <th>Action</th>
                                                  @endif
                                                </tr>
                                            </thead>
                                            <tbody>
                                            @foreach ($show_all_subcounties as $subcounty)
                                              <tr>
                                                  <td>{{ $subcounty->subcounty }}</td>
                                                  @if(in_array('Can edit subcounty', auth()->user()->getUserPermisions()))
                                                  <td>
                                                  <a href="/edit-subcounty-form/{{ $subcounty->id }}" data-widget="edit" data-toggle="tooltip" title="edit county">
                                                  <span class="btn btn-success btn-xs">edit</span></a>
                                                  @endif
                                                  @if(in_array('Can delete subcounty', auth()->user()->getUserPermisions()))
                                                  <a href="/delete-subcounty/{{ $subcounty->id }}" data-widget="deny" data-toggle="tooltip" title="delete county">
