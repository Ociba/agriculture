  ...
  <div class="content-wrapper">
    ...
    <div class="card">
            ...
            <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
                ...
                <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
                @if(in_array('Can search market products', auth()->user()->getUserPermisions()))
                <form action="/search-market-products" method="get">
               ...
               <a href="/display-market-products-form" button type="button" class="btn btn-primary">Add Market Products</button></a>
               @endif
            </div>
                </div>
            </div>
            ...
            <div class="card-body table-responsive no-padding">
              <table  class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <th>Name</th>
                  <th>Image</th>
                  @if(in_array('Can see market products action', auth()->user()->getUserPermisions()))
                  <th>Action</th>
                  @endif
                </tr>
                </thead>
                <tbody>
                @if ($show_market_products->currentPage() > 1)
                      @php($i =  1 + (($show_market_products->currentPage() - 1) * $show_market_products->perPage()))
                      @else
                      @php($i = 1)
                      @endif
              @foreach ($show_market_products as $index =>$market)
                  <tr>
                      <td>{{ $i++ }}</td>
                      <td>{{ $market->name }}</td>
                      <td>
                      <img src="{{asset('assets/images/'.$market->image)}}" style="width:60px" height="30px" alt="">
                      </td>
                      <td>
              ...
              {{ $show_market_products->appends(['name' => $search_query])->links() }}
              @else
              {{ $show_market_products->links() }}
              @endif
            </div>
            </div>
            <!-- /.card-body -->
