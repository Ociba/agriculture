  ...
  <div class="content-wrapper">
    ...
    <div class="card">
            ...
            <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
               ...
               <a href="/display-product-form" button type="button" class="btn btn-primary">Add Product</button></a>
               @endif
            </div>
                </div>
            </div>
            ...
            <div class="card-body table-responsive no-padding">
              <table  class="table table-bordered table-hover">
                ...
                <tr>
                  <th>No</th>
                  <th>Product</th>
                  @if(in_array('Can see product created by', auth()->user()->getUserPermisions()))
                  <th>Created By</th>
                  @endif
                  @if(in_array('Can see product action', auth()->user()->getUserPermisions()))
                  <th>Action</th>
                  @endif
                </tr>
                </thead>
                ...
                @if ($show_all_products->currentPage() > 1)
                      ...
                      @php($i = 1)
                      @endif
              @foreach ($show_all_products as $index =>$products)
                  <tr>
                      <td>{{ $i++ }}</td>
                      <td>{{ $products->product }}</td>
                      @if(in_array('Can see product createdby name', auth()->user()->getUserPermisions()))
                      <td>{{ $products->name }}</td>
                      @endif
              ...
              {{ $show_all_products->appends(['name' => $search_query])->links() }}
              @else
