            ...
            <div class="card-body table-responsive no-padding">
              <table  class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <th>Subject</th>
                  <th>Message</th>
                  @if(in_array('Can see message action', auth()->user()->getUserPermisions()))
                  <th>Action</th>
                  @endif
                </tr>
                </thead>
                ...
                @if ($display_contact_messages->currentPage() > 1)
                      ...
                      @php($i = 1)
                      @endif
              @foreach ($display_contact_messages as $index =>$message)
                  <tr>
                      <td>{{ $i++ }}</td>
                      <td>{{ $message->name }}</td>
                      <td>{{ $message->email }}</td>
              ...
              {{ $display_contact_messages->appends(['name' => $search_query])->links() }}
              @else
              {{ $display_contact_messages->links() }}
              @endif
            </div>
            </div>
            <!-- /.card-body -->
