  ...
  <div class="content-wrapper">
    ...
    <div class="card">
            ...
            <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
                ...
                <div class="col-lg-4 col-md-4 col-xs-4 col-sm-4">
                @if(in_array('Can search news', auth()->user()->getUserPermisions()))
                <form action="/search-news" method="get">
                        <div class="input-group ">
                          <input class="form-control"  selected="selected" placeholder="Search By news" name="news" id="srch-term" aria-label="Search" required>
            ...
            <div class="card-body table-responsive no-padding">
              <table  class="table table-bordered table-hover">
                ...
                <tbody>
                @if ($show_news->currentPage() > 1)
                      @php($i =  1 + (($show_news->currentPage() - 1) * $show_news->perPage()))
                      @else
                      @php($i = 1)
                      @endif
              @foreach ($show_news as $index =>$news)
                  <tr>
                      <td>{{ $i++ }}</td>
                      <td>{{ $news->subject }}</td>
                      <td>
                      <img src="{{asset('assets/images/'.$news->image)}}" style="width:60px" height="30px" alt="">
                      </td>
                      <td>
              ...
              {{ $show_news->appends(['name' => $search_query])->links() }}
              @else
              {{ $show_news->links() }}
              @endif
            </div>
